...
const conf = {
  ...
  whenSingleSelect(opts = {}) {
    ...
    // if (days[ idx ].showTodoLabel) days[ idx ].showTodoLabel = false;    //todo
  ...
  setTodoLabels(options = {}) {
    ...
    shouldMarkerTodoDay.concat(currentMonthTodoLabels).forEach((item) => {
      ...
      // if (target) target.showTodoLabel = !target.choosed;  //todo
      if (target) target.showTodoLabel = true;
    });
    const o = {
      'calendar.days': days,
  ...
  initSelectedDay(days) {
    ...
    daysCopy.map(item => {
      ...
      // if (todoLabelsCol.indexOf(`${item.year}-${item.month}-${item.day}`) !== -1) item.showTodoLabel = !item.choosed;    //todo
    });
    return daysCopy;
  },
  /**
  ...
  calculateNextWeekDays() {
    let {
      lastDayInThisWeek,
      lastDayInThisMonth
    } = conf.calculateLastDay.call(this);
    let {
      curYear,
      curMonth
    } = this.data.calendar;
